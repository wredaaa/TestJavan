...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	connection.Where("email = 	?", authDetails.Email).First(&users)
	if users.Email == "" {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode("Username or Password is incorrectcc")
		return
	}

	check := CheckPasswordHash(authDetails.Password, users.Password)

	if !check {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode("Username or Password is incorrectvv")
		return
	}
